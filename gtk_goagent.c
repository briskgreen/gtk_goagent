...
int main(int argc,char **argv)
{
	...
	create_menu_with_image(menu,GTK_STOCK_OPEN,accel_group,connect_goagent,text);
	create_menu_with_image(menu,GTK_STOCK_CLOSE,accel_group,disconnect_goagent,text);
	...
	open=gtk_toggle_button_new_with_label("Connect");
	gtk_box_pack_start(GTK_BOX(hbox),open,FALSE,FALSE,10);
	g_signal_connect(G_OBJECT(open),"clicked",G_CALLBACK(connect_goagent),text);
	close=gtk_button_new_with_label("Disconnect");
	gtk_box_pack_end(GTK_BOX(hbox),close,FALSE,FALSE,10);
	g_signal_connect(G_OBJECT(close),"clicked",G_CALLBACK(disconnect_goagent),text);
